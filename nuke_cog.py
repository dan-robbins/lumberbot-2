...
class nuke_cog(commands.Cog):
    ...
    async def nuke_command(self, ctx):
        voice_client = discord.utils.get(self.bot.voice_clients, guild=self.bot.get_guild(382437037314932756))
        if not (voice_client and voice_client.is_connected()):
            max_users = 0
            max_channel = None
            for channel in self.bot.get_guild(382437037314932756).voice_channels:
